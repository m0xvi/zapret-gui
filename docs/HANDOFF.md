...
**Воркфлоу (важно!):** пользователь **сам** загружает исходники в свой GitHub-репозиторий. Агент:
пишет код → проверяет сборку → **упаковывает исходники в zip** → отдаёт файл пользователю.
Не пытайтесь пушить в GitHub от себя и не просите токены. АрхиВы (zip) — предпочтительный
способ передачи: пользователь просил «не засорять хранилище агента».
...
4. Исходники отданы пользователю архивом `zapret-gui-src-v1.0.0.zip`, он загрузил их в GitHub.

**Первые шаги следующего агента:**
1. `dotnet build` + `python3 tools/check_bindings.py` + harness на реальном релизе — убедиться,
   что база в порядке (команды в §6).
2. Запросить у пользователя список правок и «те самые функции» из §9.
3. Собрать/запустить на Windows (или попросить пользователя прогнать exe и прислать журнал
   со страницы «Журнал»), устранить рантайм-замечания из §3.
4. Реализовать согласованные правки, поднять `<Version>` в csproj, обновить README и
   `docs/mockup.html` при изменении дизайна, упаковать новый zip и передать пользователю.
